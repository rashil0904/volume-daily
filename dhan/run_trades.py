...
def run_entry_321(trade_date: date | None = None, dry_run: bool = False,
    ...
    for sym in positions_today:
        print(f"[dhan] {sym} — already entered today, skipping.")
        n_skipped += 1

    ordered_symbols = [sym for sym in symbols if sym not in positions_today]
    ...
    for res in results:
        ...
        if "error" in res:
            ...
            # An order was actually placed and rejected/never filled here --
            # distinct from the earlier Phase 1 SKIPs (no ref price, 0
            # shares, insufficient balance) which never reach this branch at
            # all. Confirmed live 2026-09-07: RML's fresh 3:21pm entry was
            # rejected ("Buy back is not allowed for delivery positions...")
