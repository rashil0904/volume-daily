...
Writes  : results/trade_book.csv          (one row per position, git-diffable source of truth)
          results/trade_book.xlsx         (same data, dashboard + day-boxed visual view)
...
The xlsx has two sheets:
  Dashboard  — KPI tiles (win rate, gross/net PnL, charges) + a daily Net PnL bar chart.
  Trade Book — trades grouped into a bordered box per entry day, each row tagged
               🟢 WIN / 🔴 LOSS / ⏳ OPEN and colored to match, with in-cell data bars.
...
def _build_dashboard(wb, rows):
    ...
    header_row = 9
    for c, name in enumerate(["Entry Date", "Trades", "Wins", "Losses", "Gross PnL", "Net PnL"], start=1):
        cell = ws.cell(row=header_row, column=c, value=name)
        cell.font, cell.fill, cell.border = _BOLD, _LIGHT_BLUE, _BOX
