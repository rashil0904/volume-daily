...
def send_entry_failed(broker: str, symbol: str, error_msg: str,
                      dry_run: bool = False) -> None:
    tag  = "  [DRY RUN]" if dry_run else ""
    text = "\n".join([
        f"<b>&#128308; ENTRY FAILED — {html_lib.escape(symbol)}{tag}</b>",
        f"<b>Broker:</b> {html_lib.escape(broker)}",
        "Today's signal was skipped -- order rejected or never filled, no "
        "position was opened.",
